...
def fix_lin_chuang_questions(original_questions, docx_path):
    lines = extract_all_text(docx_path)
    
    print('Document lines:', len(lines))
    ...
    for i, line in enumerate(lines):
        ...
        if line == '单选题':
            ...
            continue
        elif line in ['三、综合题', '五、简答题', '六．名词解释', '简答题', '简答']:
            current_section = 'skip'
            continue
        
        if current_section == 'single':
            match1 = re.search(r'^(.*?)\s+([A-D])\.(.+)$', line)
            match2 = re.search(r'^(.*?)\s+([A-D])([^.\s].*)$', line)
            
            if match1:
                question_text = match1.group(1).strip()
                answer_letter = match1.group(2)
                answer_text = match1.group(3).strip()
                ...
                question_text = match2.group(1).strip()
                answer_letter = match2.group(2)
                answer_text = match2.group(3).strip()
            ...
            if ('（√）' in line or '（×）' in line or '（✓）' in line or '（✗）' in line or '（√' in line or '×）' in line) and not line.startswith('错误'):
    ...
    return js_questions, len(all_single_choice), len(all_truefalse)

if __name__ == '__main__':
    docx_path = r'E:\临时\临床医学概论(1).docx'
    
    questions, single_count, tf_count = fix_lin_chuang_questions(None, docx_path)
    
    output = f'const questions_linchuang = {json.dumps(questions, ensure_ascii=False, indent=2)};\n\nwindow.questions_linchuang = questions_linchuang;'
    
    output_path = r'E:\临时\fuxi\questions_linchuang.js'
    ...
    print("\nFirst 20 single choice questions:")
    for i, q in enumerate(questions[:20]):
        if q['type'] == 'single':
            print(f"\nQ{i+1}: {q['question']}")
            print(f"  Answer: {q['answer']} - {q['originalAnswer']}")
